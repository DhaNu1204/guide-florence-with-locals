...
function syncBookings($startDate = null, $endDate = null) {
    ...
    try {
        ...
        // Handle the booking-search endpoint response structure
        $bookings = [];
        $totalHits = 0;

        if (isset($bookingsResponse['items'])) {
            // booking-search endpoint format (confirmed by Bokun support)
            $bookings = $bookingsResponse['items'];
            $totalHits = $bookingsResponse['totalHits'] ?? count($bookings);
            error_log("Bokun Sync: Found {$totalHits} total bookings, processing " . count($bookings) . " items");
        } elseif (isset($bookingsResponse['results'])) {
            $bookings = $bookingsResponse['results'];
        } elseif (is_array($bookingsResponse) && !empty($bookingsResponse)) {
            $bookings = isset($bookingsResponse[0]) ? $bookingsResponse : [$bookingsResponse];
        }
            ...
            try {
                ...
                if ($existing) {
                    ...
                    $stmt = $conn->prepare("
                        ...
                        cancelled = ?, bokun_data = ?, last_synced = ?, updated_at = NOW()
                        WHERE id = ?
                    ");
                    $stmt->bind_param("sssssssisddssisis",
                        ...
                        $tourData['cancelled'], $tourData['bokun_data'], $tourData['last_synced'], $existing['id']
                    );
                } else {
                    // Insert new tour
                    $stmt = $conn->prepare("
                    ...
                    //        cancelled, bokun_data, last_synced)
                    $stmt->bind_param("ssssssssssisddsisiiiss",
                        ...
                        $tourData['cancelled'], $tourData['bokun_data'], $tourData['last_synced']
                    );
                }
