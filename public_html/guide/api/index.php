...
// Also handle if it's coming from /guide/api/
$endpoint = str_replace('/guide/api/', '', $endpoint);
...
if (preg_match('/(guides|tours)\/(\d+)$/', $endpoint, $matches)) {
    $resource = $matches[1];
    $id = $matches[2];
...
if (in_array($endpoint, ['guides', 'tours', 'test'])) {
