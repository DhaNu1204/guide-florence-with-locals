...
2026-09-25 step 6.14 — a “Vasari” label on Uffizi bookings sold on a Vasari Corridor rate: tours.rate_title from Bokun (productBookings[0].rateTitle, stored by the sync and backfilled: 7,361 rows, 281 Vasari), one rule (rate title contains “Vasari”); warm-gold “Vasari” chip on single rows, member rows and phone cards, “Vasari · N PAX” on group headers (people, cancelled excluded); participant sheet tags the lead name “VASARI” and adds “Vasari: X of Y PAX”. Day sheet and guide digest unchanged. — verified: vitest 332, rate_title_check 17/17, participants_check 6.14 7/7, staging sync + backfill change only rate_title (per-column diffs), digest old vs new code byte-identical, 26 Sep 10:00 = Vasari 4 PAX and 27 Sep 08:30 = none via API/UI/sheet, smoke 12/12 staging + production, July/August/September/October P&L identical to the cent.

2026-09-24 step 6.13 — a note on a merged group: “Add note for this group” in the group's Notes column (desktop row and phone card), inline editor, pale yellow note once saved; internal only (never in the guide digest); printed as “Note:” on that departure's participant sheet. Never lost: dissolve, unmerge, the sync and the 6.12 split copy it onto the bookings as “[Group note] …” (appended, never twice) or onto the new group; merging two noted groups keeps both. — verified: vitest 320, grouping_scenarios 56/56 + group_notes_http_check 19/19 on staging, a real sync left the note byte-identical, sheet and UI dissolve checked on staging, smoke 12/12 staging + production, July/August/September P&L identical to the cent.

2026-09-24 step 6.12 — auto-groups never mix languages: from 24 Sep the departure key is product + date + time + **language** (English and Italian at 14:30 = two groups, each with its own guide); a booking with no language joins only when one language is present; manual merges and past dates untouched; a mixed group shows a “Mixed languages” chip. Migration on production: 86 groups re-keyed in place, 1 split, 1 dissolved, 0 held, 0 group ids changed; next sync wrote 0 rows. Cause of the owner's 25 Sep report: the 23 Sep 23:45 cron grouped Mario (IT) with Theodoros (EN). — verified: vitest 309, grouping_check 35/35, grouping_scenarios 45/45 on the staging DB, smoke 12/12 staging + production, July/August/1–23 Sep P&L identical to the cent, upcoming guide cost ±€0.

## ✅ STEP 4.8 — A BAD SIGNAL NO LONGER LOGS HIM OUT, HANGS THE APP OR SHOWS AN EMPTY LIST (2026-09-23)
2026-09-23 step 4.9 — two serial round trips removed from every app open. With a saved login the app now draws at once and checks the login in parallel (menus from the last check; the owner-only P&L item never shows for anyone else; a dead login is sent to the login page after a glimpse of 0.3–0.8 s with no data shown), and the Tours page gets all its tour groups in one request instead of 100 at a time. Measured with a warm phone-like client through a slow-link proxy, median before → after: production Tours 2.18 → 1.50 s at 300 ms and 3.69 → 2.13 s at 700 ms (4 → 2 round trips); Tickets 1.30 → 1.15 s and 2.65 → 1.82 s (3 → 2). 4.8's hang cases unchanged or faster; vitest 301 green, smoke 12/12 on both, P&L July/August/September identical to the cent.

2026-09-23 step 6.11 — Priority Tickets gets **All museums | Uffizi | Accademia** tabs (replacing the Museum picker; All is the old page) and, on the Uffizi and Accademia tabs, a **Download participants list** button for the chosen day: one A4 sheet with every ticket booking for that museum in time order, the customer's own reference (GYG… / Viator number) large with our Bokun code small underneath, a section and subtotal per product, cancelled bookings footnoted but never printed. Also fixed: the page had been hiding every "Uffizi Gallery Reserved Ticket" booking (79 upcoming) behind an old title filter. Verified: 77 PHP checks, vitest 289 green, smoke 12/12 on both, tab = sheet on six production day/museum pairs, P&L July/August/September identical to the cent.

2026-09-23 step 4.8 — the first bad morning (owner on mobile data, `/tickets` did not load) left no recorder row and a forced re-login: the auth check hit a network error and the old code deleted a valid token, taking the row with it. Now **only a real 401 logs out**; a network error, timeout or 5xx keeps the session and re-checks quietly. **Every request has a timeout** (auth 10 s, reads 15 s + one automatic retry, full tour rows 45 s, writes 30 s never retried, PDFs 90 s, sync 180 s, route chunk 20 s, page load 5 s then the cached app) and ends in a plain message with a Retry button; a payment whose answer is lost says it **may or may not be recorded** and blocks Record until the list is refreshed. **No silent stale data**: Tours, Tickets, Priority Tickets, Payments, Guides, Radios and the Dashboard say "Could not load …" or "Could not refresh — showing data from 09:12" instead of an empty or old list. The recorder now survives a token wipe, marks every page's own data fetch, and records the verify failure reason, timeouts, automatic and user retries, and a cached-shell start. Verified: on staging with a server that never answers, verify → page at 11.7 s (still logged in), Tours list → message at 31.5 s, Tickets → last 702 tickets under the banner at 31.7 s, page navigation → app from the cached shell at 5.1 s, each with its recorder row; the previous deploy's cached shell loaded all its assets from the retained files; vitest 268 green, smoke 12/12 on both; P&L July/August/September identical to the cent.

## ✅ STEP 6.10 — THE DAILY P&L IS THE OWNER'S ALONE (2026-09-22)
2026-09-22 step 6.10 — the owner asked for `/daily-pnl` to be visible to him and nobody else. The audit found the premise was wrong in one important way: **production has two admin accounts**, `dhanu` and `sudesh` (both active), so the existing `requireRole('admin')` on `pnl.php` was never "Dhanu only". He chose an explicit **owner gate** rather than demoting the second admin, so `sudesh` keeps every other admin power and simply sees no money. `Middleware::requirePnlOwner()` = authenticated **and** admin **and** on the owner list (`PNL_OWNER_USERS` in the server .env, default `dhanu`, matched case-insensitively on username or email, so the gate is correct the moment it deploys and needs no env edit). It guards every `pnl.php` route — day view, month view, `?action=settings`, `?action=costs`, `?action=links|link|unlink` — answering **403 "You do not have access to this page."**; an unauthenticated caller still gets **401**, not 403. `pnl_links.php` is a helper, not an endpoint, and now answers **404** to a direct HTTP hit. The interface mirrors the server rather than deciding anything: `auth.php` returns `pnl_access` on login and verify, `AuthContext` exposes `canSeePnl()`, the **Daily P&L** menu item is gone for a non-owner, and `/daily-pnl` is wrapped in a new `OwnerRoute` that renders a plain **"You don't have access to this page"** card *inside* the layout — no redirect, no broken screen, no endless spinner. **No figure, rate or calculation was touched, and the numbers prove it: July 66,261.53 / 60,281.00 / 5,980.53, August 42,723.87 / 42,791.00 / −67.13, September 77,284.98 / 70,672.00 / 6,612.98 — byte-identical before and after the deploy.** Staging was attacked three ways by a real non-admin account: the menu (absent), the direct URL (access card), and every API route (403 on four GETs and four POSTs) — and a temporary session for `sudesh` proved a **second admin** is refused too while still getting 200 on tours and radios. Every P&L feature was exercised and restored on staging (rate settings, manual amount override, merge for costing, unmerge) and re-checked on production as the owner (day view, month view, Rates & Costs). **What this does NOT close, and he should know: a non-admin can still see money elsewhere** — the customer price on each booking (`bokun_total_price`, the "Customer paid" line in the booking modal, served by `tours.php` / `tour-groups.php`), every guide payment amount and the per-guide cash/bank totals on the **/payments** page (`payments.php`, `guide-payments.php`, `payment-reports.php`), and ticket prices (`tickets.php`). Guide Reports is counts only. 9 PHP checks, 7 new vitest cases, suite 219 green, smoke 12/12 on both.

## ✅ STEP 6.9 — THE VIATOR ACCOUNT SWITCH: DO NOT LOSE THE BOOKINGS HE STILL HAS TO HONOUR (2026-09-21)
2026-09-21 step 6.9 — the owner has opened a **new Viator account** and is retiring the old one, which still holds **50 live bookings across 48 departures, 143 guests, as far out as 30 May 2027** that he must still run and ticket. **Part A answered the frightening question first, and the answer was good: the sync never removes or cancels a booking merely because Bokün stopped returning it.** `syncBookings()` iterates only over what Bokün returns; there is no reconcile-missing pass in any of the three triggers (cron, “Sync Now” and the webhook all call the same function), `DELETE FROM tours` exists only in the two admin endpoints, and `cancelled` is set only from a returned booking's own status. Proved on live data: two 30 Dec 2026 rows had not been returned by Bokün for **89 days** and were untouched. Every Viator booking was exported first anyway — **2,324 rows with complete `bokun_data`, 18.1 MB, mode 600, outside the web root**, plus a CSV of the future departures. **Part B then built the one thing with a deadline**: nothing in Bokün's payload distinguishes one Viator account from another (same channel id 12296, same seller id 5194, same reference shapes, same vendor), so the marker had to be ours and had to be written before he disconnects. **All 2,324 production bookings are now stamped `legacy`** (0 unlabelled, 0 non-Viator rows labelled), and the stamp is part of creating the column so it could not be forgotten. The label is **written on INSERT only** — absent from the sync's UPDATE and from its `ON DUPLICATE KEY UPDATE`, beside `paid`/`payment_status` — so a legacy row is **incapable** of being relabelled: a real sync over 2026-09-29 on staging rewrote four legacy rows (their `last_sync` advanced, their fingerprints changed) and left every label alone, and a deleted row re-created by the sync came back `legacy` with no cutover recorded and `current` with one. A **watchdog** counts the bookings still to honour once a day and shouts if the number falls further than the departures that simply ran — baseline **50 / 48 / 143** on production, and on staging it went quiet at 49, **ALERT at 48**, quiet again at 49. He sees a grey **“old Viator”** chip on those bookings in the Tours list and the booking modal, a separate **“Viator (old account)”** line in the P&L, and a **Download the list (CSV)** button on Bokun Integration carrying Viator's own reference numbers — the thing to keep if Viator's portal stops showing him the old bookings. `booking_channel` was never touched, so **July, August and September are identical to the cent**. The one risk that remains is not technical: **once he disconnects, our copy of those 48 departures stops updating** — if Viator moves a guest to another time we will not know — so they must be checked by hand. 25 PHP checks, 4 vitest cases, suite 212 green, smoke 12/12 on both.

## ✅ STEP 6.8 — A PRINTABLE PARTICIPANT LIST FOR THE MUSEUM DOOR (2026-09-21)
2026-09-21 step 6.8 — every departure on the Tours page now has a small document icon that downloads **one A4 PDF for that departure**: a grouped or manually merged departure is one sheet, so the 10:00 and the 14:15 Uffizi are two separate downloads. The sheet carries what the museum asks for — **booking reference, every traveller name the channel gave us, PAX, channel, reselling agency, AD / CH / IN** — above it the guide with their phone number (or **NO GUIDE ASSIGNED** in place of a blank), the date, time, language and total, and below it the blank **“Tickets used:”** box he writes on by hand. Cancelled bookings are left out and counted in a footnote; a departure with nothing live still prints a sheet that says so. The button is on all four layouts (desktop row, desktop group, mobile card, mobile group card), is icon-only so a row cannot get wider on a phone, and any logged-in user can print one — the guide as well as the owner. Nothing is written, ever. **PDF engine: FPDF 1.86 vendored in `api/lib/fpdf`** (public domain, no composer, no C extension, all `.php` so the deploy allowlist ships it unchanged; `/api/lib` returns 403 over HTTP). A sheet is ~3 KB because no font is embedded, which is the point on a phone in the street — the cost is CP1252 text, so a name outside it is transliterated (**Thaís, Yildiz**) and the 8 production names that cannot be carried at all print **“(name in another script - see reference)”** rather than a row of boxes. Two faults found on staging and fixed before production, both because nothing rendered a PDF outside the endpoint: the museum came back as an array, and the sheet class declared `$w`, which collides with FPDF's own page width (a fatal, 0-byte 500). `participant_sheet.php` now holds the renderer and needs no database, so the checks build **six real PDFs** — normal, no guide, no live bookings, no museum, long (2 pages) — and assert `%PDF`, page count and size. Verified on staging across a mixed cancelled/live group, an all-cancelled tour, a hand-entered tour and a walking tour that names no museum; `m1` (a 6.2 costing merge, which is not a departure at a door) → 400, unknown unit → 404, no token → 401. **Production: nothing written, one sheet generated as proof** — tomorrow's 14:15 Uffizi & Accademia, 5 bookings, 9 pax, 3,069 bytes, one page. 39 PHP checks, 5 vitest cases, suite 208 green, smoke 12/12 on both.

## ✅ STEP 6.7 — THE CARD FEE ON DIRECT SALES (2026-09-21)
2026-09-21 step 6.7 — 6.6 stopped deducting a 30% commission the owner never pays on his own sales; this adds back the one deduction that is real, **Stripe at 1.5%**, the rate he gave. **Who pays it is read from each booking's own payment record, not guessed from the channel**: 202 direct sales carry a Stripe PaymentIntent (`STRIPE_TOKEN`, `pi_…`) and are charged; 2 voucher redemptions and 1 unpaid back-office booking are not. **An OTA booking gains nothing** — including Viator, whose commission is zero only because it reports net. The rate is a setting (**Rates & Costs → “Card processing fee — direct sales (%)”**, default 1.5, set it to 0 to switch the deduction off); nothing is hard-coded and no per-transaction cent amount was invented because he did not give one. It is named as a card fee everywhere and never as a commission: a blue **“card fee €3.59”** chip on the row and a header that reads **“Retail − commission − card fee”**. The test booking WEB-98701751 now reads **239.12 − 3.59 = 235.53**. Production moved down by exactly 1.5% of card-paid direct revenue and nothing else — **July profit 6,013.06 → 5,980.53, August −40.16 → −67.13, September 6,444.49 → 6,381.82** — with retail, commission and total cost unchanged and both manual overrides still winning. 50 PHP checks, 4 vitest cases, suite 203 green, smoke 12/12 on both.

## ✅ STEP 6.6 — DIRECT SALES NO LONGER LOSE 30% THE OWNER NEVER PAID (2026-09-21)
2026-09-21 step 6.6 — sales through **florencewithlocals.com, payment links and the back office** were having a **30% commission deducted that the owner pays to nobody**. Bokûn files a direct sale under `customerInvoice` with commission 0 rather than under `resellerInvoice`, and the code treated that as “no information” and guessed `comm_default = 30%`. **205 bookings, EUR 7,852.21 all-time, EUR 2,159.89 in the last 90 days — his profit was understated, never overstated.** The fix reads the invoice that was there all along. **The OTA path is untouched and provably unmoved**: GetYourGuide, Viator, Airbnb and Headout totals are identical to the cent before and after, because all 7,047 of their bookings carry a reseller invoice and return before the new step. Production moved upward by exactly the direct-sale commission and nothing else — **July profit 5,362.73 → 6,013.06, August −579.47 → −40.16, September 5,155.88 → 6,444.49** — with retail and total cost unchanged and both manual revenue overrides still winning. The test case from the investigation, WEB-98701751, now reads **EUR 239.12 instead of 167.38**. A guessed figure can no longer pass for a known one: the “estimated” marker is amber and says **“not from an invoice”**, and the day and month headers count how many departures are estimates — **63 → 0** over the last 90 days. The ladder now names the direct channels, adds Airbnb at its real 20% and corrects Headout to 20%, all as a safety net that nothing reaches today. The only deduction still missing on direct sales is the card-processing fee, which needs the owner’s actual rate and was deliberately not invented. 31 PHP checks, 3 vitest cases, suite 202 green, smoke 12/12 on both.

## ✅ STEP 4.7 — FIELD INSTRUMENTATION FOR THE MOBILE PROBLEM (2026-09-21)
2026-09-21 step 4.7 — Part 1 of the mobile investigation proved the app has **no timeout on any request** (a stalled connection leaves it waiting forever) and that opening the tour list costs **eight serial round trips**, but it could not say which of those the owner actually hits on a bad morning, because a Florence street cannot be simulated from a datacenter. This records what his real phone experiences: one row per page load with six timings, each request’s outcome (**including `pending` — started and never finished**), the connection type and round-trip time, whether a service worker was in control, whether it was the first load after a deploy, and a 429 counter. **It changes nothing about how the app behaves** — no timeouts, no retries, no auth or fallback changes — on purpose, so the “before” picture stays clean for Part 2B. One `navigator.sendBeacon` at the end of the load, fire-and-forget: with the endpoint returning 500, and again with it hanging, the page rendered normally both times. The test that mattered: with the tour list hanging, a row was still written (`list=pending`); with the auth check hanging — the white-screen case — a row was still written (`verify=pending, chunk=none, list=none`). A hang that produced no row would have made the whole thing useless. Admin-only **/load-measurements** shows one row per load with the phases as bars; a phase that never finished runs red to the edge. No tour or customer data is recorded — timings, statuses and connection facts only, with route identifiers stripped. 60-day retention, indexed on the timestamp, ~60–120 rows a day. 199 tests green, smoke 12/12 on both.

## ✅ STEP 6.4a — A POSSIBLE DUPLICATE MUST ALSO LOOK LIKE THE SAME TOUR (2026-09-21)
2026-09-21 step 6.4a — the “Possible duplicate” warning from 6.4 matched on date, time and PAX alone, which on a busy morning flagged **three unrelated bookings** against one hand-entered departure; a warning that is usually wrong gets ignored. The two titles must now also overlap: generic words (tour, guided, private, gallery, ticket, florence, durations) are stripped and what is left is scored with the **overlap coefficient**, so “Florence: Michelangelo's Life and Legacy 3.5 Hr Guided Tour” still matches Bokun's “Michelangelo Private Guided Tour” (1.00) while Bargello and the two Uffizi tours score 0.00. Channel was measured and left out — every false match came from the same channel as the real one, so it separates nothing. Still computed on read and never written, so the sync remains hands-off. **Staging, the real case: 3 false matches → 0**, and a genuine duplicate still flags on both rows. 67 PHP checks (was 43), suite 185 green, smoke 12/12 on both; production July P&L unchanged.

## ✅ STEP 6.4 — ADD A TOUR BY HAND (2026-09-21)
2026-09-21 step 6.4 — some GetYourGuide listings are **not connected to Bokun**, so no sync can ever produce them (proved: Bokun returns 0 hits for the owner's booking reference while a known one returns 2). An admin-only **“Add tour”** button on Tours now records such a departure by hand — date, time, product name, language, PAX, optional guide, channel (default “GetYourGuide (direct)”), the net amount the channel pays, and notes. The row carries `source='manual'` with no `product_id`, and **the sync never touches it**: `source <> 'manual'` guards both existence lookups, the grouping pass and both backfills. Proved rather than argued — a real sync of the test date left the row **byte for byte identical across all 49 columns**. Everything downstream works on it: it takes a guide, appears in the evening digest, in the radio plan (under *Altro*), in the unassigned report until it has a guide, under the language filter, and in the Daily P&L with the amount entered treated as **net** (no commission subtracted twice). Where nothing can be known — museum tickets for a title that names no museum, the guide fee when the automatic rate is 0 — the P&L prints **“—”**, never a confident €0.00. When the listing is finally connected in Bokun, the same departure starts arriving from the sync: nothing is merged automatically, both rows show **“Possible duplicate”** (same date, time and PAX) and one click removes the hand-entered copy. That match is deliberately coarse and does produce false positives on a busy morning — it only ever raises a badge. 43 PHP checks, 9 vitest cases, suite 185 green, smoke 12/12 on both; production P&L for July and September unchanged to the cent and **0 manual rows created there**.

## ✅ STEP 6.1 — FILTER TOURS BY LANGUAGE (2026-09-21)
2026-09-21 step 6.1 — a “Filter by Language” dropdown sits beside the guide filter on Tours and offers only the languages that exist in the range you are looking at, with a departure count on each. The filtering happens on the server (`tours.php` / `tour-groups.php` accept `language=`), so it works across the whole range, not just the rows already loaded; a **mixed group appears under both** of its languages and reads e.g. “English, Spanish”; rows with no language are selectable as **“Unknown”** instead of vanishing. The unassigned report now prints the language on every line. The value itself is computed once at sync time and stored: `tourLanguageCanonical()` gives one spelling per language (EN/eng/inglese → English, espanol → Spanish …) and the browser-side extractor that re-parsed `bokun_data` on every render is gone. Audit first: production already held exactly five clean values, so the normalisation changed **0 rows**; the backfill (run after the owner's confirmation, ids saved first) filled 17 past rows and left 422 as Unknown — all of them in the past, none in the upcoming list. Counts now: English 6,299 · Unknown 422 · Spanish 190 · Italian 133 · German 118 · French 90. API vs SQL verified on both environments (production English 440/440, Italian 7/7, Spanish 2/2); 44 PHP checks + 5 new vitest cases, suite 176 green, smoke 12/12 on both.

## ✅ STEP 6.3 — THE RADIO ORDER WRITES ITSELF (2026-09-21)
2026-09-21 step 6.3 — the afternoon WhatsApp to Vox Firenze is now generated by the app. A new admin-only **Radios** page shows tomorrow's departures grouped by museum in exactly the format the owner types by hand — one line per departure, two departures at the same time as two lines, times without a leading zero, guests only (Vox adds the guide's transmitter), “Ciao,” before 15:00 Rome and “Buonasera,” after, “Grazie” at the end — with the message in an editable box, Copy and Download .txt, a detail table (museum, time, product, guide, PAX) to check it against reality, and “Mark as sent” recording what went out in the new `radio_orders` table. The museum mapping was shown to the owner and agreed before any UI was written: a tour that names a museum gets that heading (Bargello and Palazzo Vecchio included), one that names none goes in a catch-all “Altro” section last, and a combo meets at the first museum in its title — confirmed by the data (22 Sep really has two 9:30 combo departures of 9 and 7 PAX, the shape of his own example message). Ticket products and cancelled bookings produce no line; a departure with no guide still gets receivers but no transmitter. 27 PHP checks including both of his real example messages reproduced character for character, vitest 171, smoke 12/12 on both, viewer 403 / no token 401. Nothing was sent anywhere and production has 0 recorded orders. The older 6.3 sketch (slot curves, peak concurrency, supplier unit costs, P&L radio_cost link) was deliberately not built.
