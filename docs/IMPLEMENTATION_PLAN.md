...
- `git rm --cached` + `git filter-repo --invert-paths` for: `fix_users.php`, `server/db.js`, `backend/`, `tours_temp.json`, `database_backup_before_payment_system.sql`, `tickets_import.csv`, `RE Your Bókun support case 00301047.txt`, `product_details.txt`, `getyourguide_data.txt`, both stale `public_html` builds. Strip all `INSERT` rows from `database_schema_updated.sql`. Remove passwords from `CLAUDE.md` / `docs/API_DOCUMENTATION.md`. Force-push; re-clone.
