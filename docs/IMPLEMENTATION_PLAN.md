...
- **URGENT, needs the owner's go:** the production web root still serves old one-off files with HTTP 200: `/CLAUDE.md` (Feb-2026 copy, contains the admin password), `/docs/API_DOCUMENTATION.md` (same), `/test.php`, `/setup_database.php`, `/migrate_production_database.php`, `/check_tables.php`, `/check_payments_structure.php`, `/create_views_*.php`, `/execute_payment_views.php` (`.sql` files are already 403). They are all inside `~/backups/pre-0.2-20260910_215631.tgz`. Proposed action: move them to `~/backups/prod-root-leftovers-<date>/` (Claude Code's attempt was blocked by its permission policy). Also extend the root `.htaccess` deny to `*.md` and the maintenance-script name families (like the api one).
