...
// Use a simple relative path instead of hardcoded domain or environment variable
...
export const updateTourPaidStatus = async (tourId, isPaid) => {
  try {
    ...
    const response = await axios.put(`${API_BASE_URL}/tours/${tourId}/paid`, { paid: isPaid });
    ...
    try {
      ...
      const retryResponse = await axios.put(`${API_BASE_URL}/tours/${tourId}/paid`, { paid: isPaid });
      await getTours(true);
...
export const updateTourCancelStatus = async (tourId, isCancelled) => {
  try {
    // Use the correct API endpoint path to match our backend implementation
    const response = await axios.put(`${API_BASE_URL}/tours/${tourId}/cancelled`, { cancelled: isCancelled });
    ...
    try {
      console.log('Retrying with alternate API path...');
      const retryResponse = await axios.put(`${API_BASE_URL}/tours/${tourId}/cancelled`, { cancelled: isCancelled });
      await getTours(true);
      return retryResponse.data;
    } catch (retryError) {
      console.error('Retry also failed:', retryError);
