...
- [x] Guide phone validation (2026-06-29) — Add/Edit Guide form (Guides.jsx) now blocks non-international numbers that would break WhatsApp reminders. `isValidGuidePhone()` strips spaces/dashes/dots/parens then requires `/^(\+|00)\d{8,15}$/` (must start with `+` or `00` country code, then 8–15 digits; empty = invalid). Inline terracotta warning + toast on submit; bare local numbers (e.g. `3392863290`) and emails are rejected. See CLAUDE.md → "Guide phone validation (Jun 2026)".
- [x] Tour grouping by product_id (2026-06-29) — auto-grouping now keys on **Bokun product_id + date + HH:MM** (was normalized title), so the same tour sold under different channel titles (e.g. product 962885's "Uffizi & Accademia Walking Tour…" and "Uffizi, David Tour & Gelato…") groups into ONE departure. `display_name` = most-frequent title among the group's bookings (tie → most PAX). `autoGroupAfterSync` in bokun_sync.php; manual merges (`is_manual_merge`) untouched. See CLAUDE.md → "Tour Grouping (Jun 2026)".
- [x] Private tours — product-aware classification (2026-06-29) — `tours.is_private` flag set on every sync from `tour_classification.php` (single source of truth): fully-private products are always private; mixed products are private only for specific rate IDs (rateId read from `bokun_data` → `productBookings[0].fields.rateId`). Private tours show a purple **Private** badge, render standalone, and are **never auto-grouped**. Backfill flagged 109 existing rows private. See CLAUDE.md → "Tour Classification (Jun 2026)".
- [x] Per-tour capacity (2026-06-29) — `getMaxPaxForTitle()`: Uffizi & Uffizi-combos = **9**, pure Accademia/David = **19**, else 9. Drives the group **FULL** badge and the manual-merge guard (replaces the hard-coded 9); stored on `tour_groups.max_pax`. Frontend mirror in `src/utils/tourCapacity.js`.
- [x] Cancelled bookings excluded from PAX (2026-06-29) — cancelled bookings no longer count toward a group's PAX total, the bookings-count badge, or the FULL badge (still shown struck-through in the expanded list). Active-only counts via `countActivePax`/`countActiveBookings` (tourCapacity.js) across desktop + mobile.
- [x] Assigned-tour green background (2026-06-29) — rows/groups/cards with a guide assigned (and not cancelled) get a light-green background; priority **cancelled red > assigned green > default**; drag-over/selection styles still override.
- [x] Tours date filter redesign (2026-06-29) — replaced the native `<input type="date">` + loose period buttons with a themed in-app calendar (`src/components/DateFilter.jsx`) + a segmented control. Custom popover (date-fns grid), terracotta selected day, today ring, outside-click/Escape close; **month arrows jump to the 1st** of the adjacent month. See CLAUDE.md → "Tours date filter (Jun–Jul 2026)".
- [x] Guide reminder delivery fix (2026-06-30) — reconcile was **self-cancelling every reminder ~12 min before SendAt**: Twilio's 15-min scheduling floor wrongly gated RETENTION as well as creation, so imminent reminders dropped out of the kept set and the removal pass cancelled them. Fixed via pure unit-tested `reminderPlan()` splitting **WANTED** (valid phone + within 7-day window → decides retention; never cancels imminent reminders) from **CREATABLE** (sendAt ≥ now+15min → only gates new scheduling); the removal pass cancels only NOT-wanted reminders. Plus: **one reminder per departure** (candidate query collapses each group to its lowest active tour id — grouped guides get ONE WhatsApp); group-level guide assignment propagates guide_id to member tours and reconciles.
- [x] Cancelled bookings excluded from tour COUNTS (2026-07-02) — day-header/period-header/Summary counts now count only active tours (was: PAX excluded cancelled but tour counts included them). `computeItemStats()` in Tours.jsx; counts render "N tours · M PAX" + muted "· N cancelled" only when non-zero.
- [x] Category Summary panel on Tours page (2026-07-02/03) — Summary card buckets each active departure once via `tourCategory()` (`src/utils/tourCapacity.js`, mirrors backend classifyTourCategory): Combo / Uffizi / Accademia / Pitti / Other + Private mirrors (Private Combo/Uffizi/Accademia/Pitti/(other)); 0-buckets hidden, Private tiles purple.
- [x] Adults/children/infants PAX breakdown (2026-07-03) — server-computed `pax_adults`/`pax_children`/`pax_infants` on every tour from tours.php AND tour-groups.php member rows (`computePaxBreakdown()` in tour_classification.php, from `priceCategoryBookings`); frontend `getPaxBreakdown()` prefers the server fields (fixes grouped member rows that lack bokun_data). Shown muted next to PAX only when children/infants > 0 (e.g. "5 adults, 1 child"). Booking Details modal compacted in the same pass (all fields kept).
- [x] Fix: unassigning a guide (2026-07-06) — clearing to "Unassigned" failed with "No fields to update" because PHP `isset()` is false for a present-but-null `guide_id`; fixed with `array_key_exists()` in tours.php PUT + tour-groups.php updateGroup (NULL now propagates to group member tours; reminder reconcile runs on unassign and cancels the guide's WhatsApp reminder). See CLAUDE.md → "Guide unassign fix (Jul 2026)".
- [x] Date filter buttons: Today / Tomorrow / Upcoming / Date Range (2026-07-06) — removed "Past 40 Days", added "Tomorrow" (single-date mode, tomorrow at midnight; active when the selected date is tomorrow).
- [x] Mixed merged groups flagged (2026-07-06) — Tours page: gold **Mixed** badge on group headers whose non-cancelled bookings span >1 category (tooltip breakdown) + per-booking **Type** badge in expanded rows (Combo gold). Guide Reports: `buildComposition()` in guide-tour-report.php classifies group units by MEMBER bookings — a mixed group stays 1 tour unit but counts as **"Mixed"** with `composition_label` ("Combo ×2, Uffizi ×1") and its own Mixed line in summary_by_category + the all-guides overview; PDF/CSV exports include it. Display/report only — no payment calculation touched. See CLAUDE.md → "Mixed merged groups (Jul 2026)".
